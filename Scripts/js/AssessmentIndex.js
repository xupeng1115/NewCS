	...
	methods:{
		...
		submitBtn:function(){
			if(!app.lokinKey){
				...
				app.dirResult(resultData,ResultPictrue);
				app.examShow=false;
				app.resultShow=true;
			}
			
			console.log(app.submitList);
			if(app.submitList.length<assessmentList.length){
				alert("还有"+(assessmentList.length-app.submitList.length)+"未答，请完善答案");
				return false;
			}
		...
		dirResult:function(result,resultPicture){
			...
			$.each(resultPicture,function(index,item){
				titles.push(index);
				scores.push(item);
			})
...
$(function(){
	...
	(function(){
		...
		radialObj = radialIndicator('#indicatorContainer', {
		    ...
		    initValue:0,
		    minValue:0,
			maxValue: assessmentList.length,
		...
		       	return value+"/"+assessmentList.length;
		   	}
		});
